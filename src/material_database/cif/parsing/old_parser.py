...
from material_database.cif.parsing.utils import safe_str2float, str2float
    ...
    def _get_structure(
        ...
        for idx, atom_site in enumerate(atom_sites):
            ...
            if not check_occu or occupancy > 0.0:
                print("Occupancy greater than 0")
        ...
        if (
            ...
            if symmetrized:
                try:
                    ...
                    print("Parsed wyckoff letters:", wyckoffs)
                    print("Equivalent indices:", equivalent_indices)
...
def check(
    ...
    Returns:
        str | None: If any check fails, return a human-readable str for the
            reason (e.g., which elements are missing). None if all checks pass.
    """
    cif_as_dict = cif_parser.as_dict()
    head_key = next(iter(cif_as_dict))
...
if __name__ == "__main__":
    ...
    structs = parser.parse_structures(
        on_error="raise", symmetrized=True, check_occu=True
    )
